  ...
  describe('profiles', () => {
    describe('list', () => {
      ...
      test
        .do((ctx) => {
          ctx.userConfig = new ConfigData();
          ctx.userConfig.addProfile('profile1', constants.FAKE_ACCOUNT_SID, '', constants.FAKE_API_KEY, constants.FAKE_API_SECRET);
          ctx.userConfig.addProfile('profile2', constants.FAKE_ACCOUNT_SID, '', constants.FAKE_API_KEY, constants.FAKE_API_SECRET);
        })
        .twilioCliEnv(Config)
        .stdout()
        .stderr()
      ...
      test
        ...
        .stderr()
        .twilioCommand(ProfilesList, [])
        .it('runs profiles:list with multiple profiles in profiles and projects', (ctx) => {
          expect(ctx.stdout).to.contain('profile1');
          expect(ctx.stdout).to.contain('profile2');
          expect(ctx.stdout).to.contain(constants.FAKE_ACCOUNT_SID);
          expect(ctx.stdout).to.not.contain('Region');
